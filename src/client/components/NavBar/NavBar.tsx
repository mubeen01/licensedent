      ...
      <header className={cn('sticky top-0 z-50 transition-all duration-300', isScrolled && 'top-4')}>
        <div
          className={cn('transition-all duration-300', {
            'mx-4 md:mx-20 pr-2 lg:pr-0 rounded-full shadow-[0_8px_30px_-8px_hsl(var(--foreground)/0.18)] bg-background/80 backdrop-blur-xl border border-border/80 ring-1 ring-white/10':
              isScrolled,
            'mx-0 bg-background/70 backdrop-blur-xl border-b border-border/70': !isScrolled,
          })}
        >
          <nav
          ...
          >
            <div className='flex items-center gap-6'>
              ...
              >
                ...
                <span
                  className={cn('font-bold leading-6 text-foreground transition-all duration-300', {
                    'ml-2 text-base': !isScrolled,
...
function renderNavigationItems(
  ...
  const menuStyles = cn({
    'block rounded-lg px-3 py-2 text-sm font-medium leading-7 text-foreground hover:bg-accent hover:text-accent-foreground transition-colors':
      !!setMobileMenuOpen,
    'rounded-full px-3.5 py-1.5 text-sm font-medium leading-6 text-foreground/80 duration-200 ease-in-out hover:bg-muted hover:text-foreground transition-colors':
      !setMobileMenuOpen,
  });
...
const NavLogo = ({ isScrolled }: { isScrolled: boolean }) => (
  <span
    ...
    aria-hidden='true'
  >
    <img src='/logo/licensedent-icon.svg' alt='' width={512} height={512} className='size-full object-cover' />
  </span>
);
