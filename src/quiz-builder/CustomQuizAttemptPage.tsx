import { AlertTriangle, CheckCircle2, Flag, Timer, Wand2 } from 'lucide-react';
import { useEffect, useMemo, useRef, useState } from 'react';
import { useNavigate, useParams } from 'react-router';
import { type AuthUser } from 'wasp/auth';
import { getCustomQuizAttempt, saveCustomQuizAnswer, submitCustomQuizAttempt, useQuery } from 'wasp/client/operations';
...
// Same engine as MockExamAttemptPage.tsx (timer, palette, autosave, auto-
// submit-on-timeout) -- just pointed at CustomQuizAttempt instead of a fixed
// MockTest-backed attempt. See schema.prisma's CustomQuizAttempt comment for
// why this isn't just MockExamAttempt with an optional mockTestId.
function CustomQuizAttemptPage({ user }: { user: AuthUser }) {
  const { attemptId } = useParams<{ attemptId: string }>();
  const navigate = useNavigate();
  const { data: fetched, isLoading, error: fetchError } = useQuery(getCustomQuizAttempt, { attemptId: attemptId! });
  ...
  useEffect(() => {
    if (fetched?.status === 'submitted') {
      navigate(`/quiz-builder/${attemptId}/results`, { replace: true });
    }
  ...
  const deadline = useMemo(
    () =>
      fetched && fetched.durationMinutes
        ? new Date(fetched.createdAt).getTime() + fetched.durationMinutes * 60_000
        : null,
    [fetched]
  );
  ...
  async function handleSubmit() {
    ...
    setSubmitError(null);
    try {
      await submitCustomQuizAttempt({ attemptId });
      navigate(`/quiz-builder/${attemptId}/results`);
    } catch (e: any) {
      ...
      <div className='sticky top-0 z-10 border-b border-border bg-background/95 backdrop-blur-sm px-4 py-3 flex items-center justify-between gap-4'>
        <div>
          <p className='text-sm font-bold text-foreground flex items-center gap-1.5'>
            <Wand2 className='h-3.5 w-3.5 text-primary' /> Custom Quiz
          </p>
          ...
          <div className='grid grid-cols-8 lg:grid-cols-6 gap-1.5'>
            {items.map((item, i) => {
              ...
              return (
                ...
                >
                  {i + 1}
                  {a?.markedForReview && <span className='absolute -top-1 -right-1 h-2 w-2 rounded-full bg-gold' />}
                </button>
              );
            })}
          </div>
...
export default CustomQuizAttemptPage;
