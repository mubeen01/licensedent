...
function CustomQuizResultsPage({ user }: { user: AuthUser }) {
  const { attemptId } = useParams<{ attemptId: string }>();
  const { data, isLoading, error } = useQuery(getCustomQuizResults, { attemptId: attemptId! });
  const [filter, setFilter] = useState<Filter>('all');

  return (
    <DashboardLayout user={user} pageTitle='Quiz Results'>
        ...
        {data && (
          <>
            <div className='rounded-sm border border-border bg-gradient-to-br from-primary via-primary to-secondary text-primary-foreground shadow-lg p-6 md:p-8 flex flex-col items-center text-center gap-2'>
              ...
              <h2 className='text-lg font-bold'>Custom Quiz</h2>
...
export default CustomQuizResultsPage;
