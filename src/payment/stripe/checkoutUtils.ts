...
export async function createStripeCheckoutSession({
  ...
  try {
    return await stripe.checkout.sessions.create({
      ...
      customer: customerId,
      // Read back in the webhook (checkout.session.completed) to set Subscription.examAccessId
      // for single-exam plans -- see payment/plans.ts's PaymentPlanEffect 'access' case.
      metadata: examId ? { examId } : undefined,
    });
  } catch (error) {
    console.error(error);
    throw error;
