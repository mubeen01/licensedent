...
// Matches schema.prisma's SubscriptionPlanType enum (fast_track/standard/extended) — these
// are one-time, fixed-duration exam-access passes, not recurring subscriptions.
...
export type PaymentPlanEffect =
  ...
  // which sets the Subscription row's `examAccessId` (see stripe/webhook.ts).
  | { kind: 'access'; durationDays: number; allExamsAccess: boolean };
